...
public enum Instruction {
	...
	public final static EnumMap<Instruction, Consumer<Integer[]>> execs = new EnumMap<>(
		Map.ofEntries(
			...
			Map.entry(BEQ,		args -> { if (R[rs] == R[rt]) PC[0] += args[imm]; }),
			Map.entry(BNE, 		args -> { if (R[rs] != R[rt]) PC[0] += args[imm]; }),
			Map.entry(J, 		args -> PC[0] += args[addr]),
			Map.entry(JAL, 		args -> { R[31] = PC[0] + 1; PC[0] = args[addr]; }),
			Map.entry(JR,		args -> PC[0] = R[rs]),
			Map.entry(LBU,		args -> R[rt] = memGet(R[rs] + args[imm]) & 0x000000FF),
			Map.entry(LHU,		args -> R[rt] = memGet(R[rs] + args[imm]) & 0x0000FFFF),
			Map.entry(LL,		args -> R[rt] = memGet(R[rs] + args[imm])),
			Map.entry(LUI,		args -> R[rt] = memGet(R[rs] + args[imm]) & 0xFFFF0000),
			Map.entry(LW,		args -> R[rt] = memGet(R[rs] + args[imm])),
			Map.entry(NOR,		args -> R[rd] = ~(R[rs] | R[rt])),
			Map.entry(OR,		args -> R[rd] = R[rs] | R[rt]),
			Map.entry(ORI,		args -> R[rt] = R[rs] | args[imm]),
			Map.entry(SLT, 		args -> R[rt] = (R[rs] < R[rt]) ? 1 : 0),
			Map.entry(SLTI,		args -> R[rt] = (R[rs] < args[imm]) ? 1 : 0),
			Map.entry(SLTIU,	args -> R[rt] = (R[rs] < args[imm]) ? 1 : 0),
			Map.entry(SLTU,		args -> R[rd] = (R[rs] < R[rt]) ? 1 : 0),
			Map.entry(SLL,		args -> R[rd] = R[rt] << args[shamt]),
			Map.entry(SRL,		args -> R[rd] = R[rt] >>> args[shamt]),
			Map.entry(SB,		args -> memSet(R[rs] + args[imm], memGet(R[rs] + args[imm]) & 0xFFFFFF00 + R[rt] & 0x000000FF)),
			Map.entry(SC,		args -> memSet(R[rs] + args[imm], Processor.atomic ? 1 : 0)),
			Map.entry(SH,		args -> memSet(R[rs] + args[imm], memGet(R[rs] + args[imm]) & 0xFFFF0000 + R[rt] & 0x0000FFFF)),
			Map.entry(SW,		args -> memSet(R[rs] + args[imm], R[rt])),
			Map.entry(SUB,		args -> R[rd] = R[rs] - R[rt]),
			Map.entry(SUBU,		args -> R[rd] = R[rs] - R[rt])
		)
	);
